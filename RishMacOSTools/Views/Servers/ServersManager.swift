...
enum ServersManager {
    ...
    static func ensureSSHScaffold() -> Bool {
        ...
        let folderURL = StaticHelper.sshFolderUrl
        ...
        do {
            ...
            StaticHelper.showAlert(message: "Failed to prepare ~/.ssh: \(error.localizedDescription)", error: true)
            return false
        }
    }
    ...
    static func getServers(force: Bool = false) -> [ServerObject] {
        if servers.isEmpty || force {
            ...
            if FileManager.default.fileExists(atPath: configPath) {
                do {
                    let lines = try String(contentsOfFile: configPath, encoding: .utf8).components(separatedBy: .newlines)
                    for line in lines {
    ...
    static func createServer(host: String, hostname: String, user: String, key: String, addKeysToAgent: Bool = true) -> Bool {
        ...
        if servers.contains(where: { $0.host.lowercased() == normalizedHost }) {
            StaticHelper.showAlert(
        ...
        var server = ServerObject(host: normalizedHost)
    ...
    static func pullServersToFile() -> Bool {
        ...
        do {
            ...
            let backupURL = URL(fileURLWithPath: configBacPath)
            let tmpURL = configURL.deletingLastPathComponent().appendingPathComponent("config.tmp")
    ...
    static func exportProfiles(profiles: [ServerObject]) {
        ...
        let profileData: AnyEncodable = profiles.count == 1
        ? AnyEncodable(ITermProfileObject(server: profiles[0]))
        : AnyEncodable(["Profiles": profiles.filter { $0.host != "*" }.map { ITermProfileObject(server: $0) }])
